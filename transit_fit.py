...
def transit_emcee(diz, ind):
    ...
    bounds_model.append((0.08, 0.12))  # rp/r*
    params.add('t0', value=0.09, min=0.08, max=0.12)
    ...
    bounds_model.append((1e-3, 0.015)) # sigma
    params.add('sigmaSpot', value=0.01, min=1e-3, max=0.02)
    #bounds_model.append((None, None)) # exp Spot
    bounds_model.append((0.1, 0.11))  # rp/r*
    params.add('x0', value=0.09, min=0.08, max=0.12)
    if wl > 2.5:
    ...
    if wl <= 2.5:
        ...
        bounds_model2.append((0.08, 0.12)) # t0
        bounds_model2.append((-0.5, 0.5))    #r0
        bounds_model2.append((0., 2.))    # r1
        bounds_model2.append((1e-4, 1.)) # A
        bounds_model2.append((1e-3, 0.015)) # sigma
        bounds_model2.append((0.10, 0.11)) # x0
        initial_params = kr, t0, r0, r1, A, sigma, x0
        soln = minimize(nll, initial_params, jac=False, method='L-BFGS-B', \
...
def plot_best_white(sol, t, y, yerr, datasav, \
    ...
    if chi2(yTh_t, y, yerr)/(len(y) - len(params)) > 3.:
        set_trace()
...
def transit_white(par, t):
    ...
    #print(aR)
    params = batman.TransitParams()
    params.t0 = par[1] #par[1]                #time of inferior conjunction
    params.per = per_planet           #orbital period
    params.rp = par[0]                #planet radius (in units of stellar radii)
...
def transit_syst_spot(par, t):
    ...
    if wl <= 2.5:
        ...
        model = (transit_white([kr, t0, q1, q2], t) \
                    + gauss(t, [Aspot, sigmaSpot, x0]))*np.polyval([r0, r1], t)
    else:
        kr = par[0]
        t0 = par[1]
        r0 = par[1]
        ...
        sigmaSpot = par[4]
        x0 = par[5]
        model = (transit_white_noLD([kr, t0], t) \
                    + gauss(t, [Aspot, sigmaSpot, x0]))*np.polyval([r0, r1], t)

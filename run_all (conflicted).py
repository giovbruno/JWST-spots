...
def plot_res(inputpars, mags, tcontrast, models, fittype):
    ...
    for mod in [models]:
        for i, mag in enumerate(mags):
            ...
            for j, td in enumerate([-1200.]):#tcontrast):
                tumbra = ip['tstar'] + td
        ...
        ax.text(-1420, 9, r'True $T_\mathrm{spot} - T_\mathrm{star}$', \
                        fontsize=14)
        axins.text(-500, 1.5, '$99.7\%$ CI', fontsize=14)
...
def main():
    ...
    for m, instrum in enumerate(['NIRCam']): #'NIRCam'
        for i, asize in enumerate([3.]):
            for j, incl in enumerate([90.]):
                for k, theta in enumerate([40., 0.]): # mu angle 0.
                    inputpars = {}
                    ...
                    #    inputpars=inputpars, update=False)
